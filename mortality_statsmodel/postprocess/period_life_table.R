#' Period life table for age groups 0-1, 1-4, 5-9, ..., 80-84, 85+
#'
#' Calculates the average number of years lived by those who die in each age group 70-74, 75-79, 80-84, 85+.
#'
#' @param lx for 70 and older, 4 rows (one for each age group)
#'
#' @return matrix of same dimensions as lx70
#'
#' @author Vasilis Kontis, \email{konvas@@gmail.com}
#' @references Thatcher AR, Kannisto V and Vaupel JW (1998). The force of mortality at ages 80 to 120.
#'
#'
#' @export
#'
...
#' For age bins 0, 1-4 calculate the ax. See p.48 Preston et al, Demography: measuring and modeling
#' population processes, 2001. "The lower the mortality, the more heavily will infant deaths be
#' concentrated at the earliest stages of infancy". The contingency table is based on Coale and
#' Demeny (1983), who fitted a line to international and intertemporal data
#'
#' @param mx mortality rates for ages 0 (mx[1,]), 1-4 (mx[2,])
#' @param 1 for male, 2 for female, fifferent sexes have different ax
#'
#' @return matrix of adjusted 1a0, 4a1
#'
#' @author Theo Rashid, \email{theoaorashid@@gmail.com}
#' @references Preston et al. (2001). Demography: measuring and modeling population processes.
#' @references Coale and Demeny (1983). Regional Model Life Tables and Stable Populations
#'
#'
#' @export
#'
...
#' Period life table for age groups 0-1, 1-4, 5-9, ..., 80-84, 85+
#'
#' @param age, vector of age groups, 0, 5, 10, ..., 80, 85 (may be repeated if more than 1 years of data are available)
#' @param mx, mortality rates corresponding to ages in "age"
#' @param ax, average number of years lived by those who die in each age group. NA values are replaced by 2.5. The ax
#' for the open-ended age group 85+ is calculated using the Kannisto-Thatcher method
#' @param sex, argument for the .young_age_extension() method
#' @param check.conv, if TRUE, it will test that the iterative procedure to estimate ax (see above) has converged
#' @param full.table, if TRUE, will return extended table
#'
#' @return data.frame(ax, mx, qx, ex, Tx, Lx, lx, age) if full.table, else data.frame(mx, qx, ex)
#'
#'
#' @author Vasilis Kontis, \email{konvas@@gmail.com}
#'
